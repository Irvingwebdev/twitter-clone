...
export const createPost = async (req, res) => {
  try {
    ...
    if (!text && !img) {
      return res
        .status(400)
        .json({ error: "You must provide a text or an image" });
    }
...
export const likeUnlikePost = async (req, res) => {
  try {
    ...
    console.error("Error in postLikeUnLike controller", error.message);
    res.status(500).json({ error: error.message });
  }
};
